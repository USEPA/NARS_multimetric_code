...
write_csv(v_in_redund, paste0(here::here(), "/Testing/Redund_test_testdata.csv"))

# Data for comparison
v_in_comp <- v_in_redund |> 
  inner_join(sites, by = 'UID')

write_csv(v_in_comp, paste0(here::here(), "/Testing/Boxplot_comp_test_testdata.csv"))

# Data for signal-to-noise
v_in_sn <- veg_met |> 
  select(UID, SITE_ID, TOTN_SPP, TOTN_NATSPP, FQAI_ALL, PCTN_HSEN, TOTN_ADVSPP)

write_csv(v_in_sn, paste0(here::here(), "/Testing/SN_test_testdata.csv"))
