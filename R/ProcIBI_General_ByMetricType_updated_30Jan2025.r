#' @title Construct and evaluate MMI based on metric types
#' @description
...
#' @param metList Data frame with column of metrics (METRIC) and a column 
...
prIBI_byType <- function(df, 
  ...
  df_2 <- df_1[complete.cases(df_1[,metList$METRIC]),]
  ...
  met_samp <- matrix(rep(' ', nummets*nsamp), ncol=nummets)
